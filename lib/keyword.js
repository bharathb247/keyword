...
function suiteKeyword(name, keywordsToRun) {
    keywords[name] = createKeywordWrapper(keywordsToRun);
}

function libKeyword(name, fn) {
...
var keyLib = pat()
    ...
    .caseof(validators.isKeywordName, _.isFunction, libKeyword)
    .caseof(validators.isKeywordName, validators.isRun, function(name, keys) {
        suiteKeyword(name, keys);
    })
    .otherwise(function() {
...
key.formats.excel = require('../formats/excel');
